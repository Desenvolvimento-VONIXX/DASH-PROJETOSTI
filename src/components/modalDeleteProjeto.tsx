...
export const DeleteProjeto: React.FC<Props> = ({ open, setOpen, idProjeto, refetch }) => {
    const handleDelete = async () => {
        try {
            if (idProjeto) {
                await JX.deletar('AD_PROJETOSTI', [{ ID: idProjeto }])
                    .then((response: any) => {
                        if (response.status == 1) {
                            ...
                            const error = response.statusMessage || "erro ao excluir o projeto.";
                            toast.error(`Ocorreu um erro ao excluir o projeto: ${error}`);
                        }
                    })
            }
        ...
        <AlertDialog open={open} onOpenChange={setOpen}>
            <AlertDialogContent className="max-w-md p-6 rounded-lg shadow-lg bg-white">
                <AlertDialogHeader className="flex flex-col items-center">
                    ...
                    <AlertDialogTitle className="text-lg font-semibold text-gray-800 mt-4">Tem certeza que deseja excluir o projeto <strong>#{idProjeto}</strong>?<br />
                    </AlertDialogTitle>
